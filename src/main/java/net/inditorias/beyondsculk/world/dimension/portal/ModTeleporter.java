...
public class ModTeleporter implements ITeleporter {
    ...
    public Optional<BlockUtil.FoundRectangle> getExistingPortal(BlockPos pos) {
        ...
        Optional<PoiRecord> optional = poiManager.getInSquare((poiType) ->
                poiType.get().equals(ModPOIs.RESONANT_PORTAL.get()), pos, 64, PoiManager.Occupancy.ANY).sorted(Comparator.<PoiRecord>comparingDouble((poi) ->
    ...
    public Optional<BlockUtil.FoundRectangle> makePortal(BlockPos pos, Direction.Axis axis) {
        ...
        BlockState resonant_portal = ModBlocks.RESONANT_PORTAL_BLOCK.get().defaultBlockState().setValue(ResonantPortalBlock.AXIS, axis);

        for(int j2 = 0; j2 < 2; ++j2) {
            for(int l2 = 0; l2 < 3; ++l2) {
                mutablePos.setWithOffset(blockpos, j2 * direction.getStepX(), l2, j2 * direction.getStepZ());
                this.level.setBlock(mutablePos, resonant_portal, 18);
            }
        }
    ...
    public PortalInfo getPortalInfo(Entity entity, ServerLevel level, Function<ServerLevel, PortalInfo> defaultPortalInfo) {
        boolean destinationIsUG = level.dimension() == ModDimensions.OTHERWORLD_DIM_KEY;
        if (entity.level.dimension() != ModDimensions.OTHERWORLD_DIM_KEY && !destinationIsUG) {
            return null;
        }
        else {
    ...
    protected Optional<BlockUtil.FoundRectangle> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if(existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.level.getBlockState(entity.portalEntrancePos).getOptionalValue(ResonantPortalBlock.AXIS).orElse(Direction.Axis.X);
